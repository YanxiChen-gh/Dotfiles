...
For any plan, design doc, or pre-implementation review artifact, default to a **Lavish HTML artifact** to open and annotate in the browser (`npx -y lavish-axi <file>`), not a plain markdown file - make it rich (sections, diagrams, comparisons, decision inputs), then `npx -y lavish-axi poll` for feedback and iterate. Fall back to markdown only when Lavish is unavailable or it's a throwaway one-liner.
...
On work machines, shared skills are symlinked into `~/.codex/skills/`, each a directory with a `SKILL.md`. Codex does not auto-load them, so if that directory exists, check it for what's available and open the relevant `~/.codex/skills/<name>/SKILL.md` when a task matches one, rather than assuming a fixed set.
