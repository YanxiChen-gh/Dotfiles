...
### Shared global skills (Claude + Cursor + Codex)

Skills under `shared-skills/` are symlinked to `~/.claude/skills`, `~/.cursor/skills-cursor`, and `~/.codex/skills` when `WORK_MACHINE=1`, so one copy works in all three tools. Claude and Cursor auto-load them; Codex does not, so `codex/AGENTS.md` points at `~/.codex/skills/` and the agent reads the relevant `SKILL.md` on demand. Cursor-only meta-skills stay under `cursor/skills/`.
