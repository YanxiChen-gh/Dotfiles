...
`prefix+a` opens a repository-first task-workspace picker. It lists the current primary repository, sibling repositories, and direct child Git repositories under the optional colon-delimited `HERDR_REPO_ROOTS`. Linked and Treehouse worktrees collapse into their canonical primary repository entry. `Open local path...` accepts any existing Git checkout, while `Clone GitHub repository...` resolves a GitHub reference, clones into the current repository's parent or another configured repo home, and continues through the same checkout setup.
