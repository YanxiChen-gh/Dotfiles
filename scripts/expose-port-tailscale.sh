...
if ! pgrep -x tailscaled >/dev/null; then
    echo "[expose-port] starting tailscaled (userspace networking)..."
...
if ! tailscale status --self --peers=false >/dev/null 2>&1; then
    ...
    "$JOIN_SCRIPT" "yanxi-$(hostname | tr -cd 'a-zA-Z0-9-' | cut -c1-40)"
fi
...
if sudo tailscale serve status 2>/dev/null | grep -q ":${TAILNET_PORT}"; then
    echo "[expose-port] replacing existing :${TAILNET_PORT} mapping (would shadow it otherwise)"
    sudo tailscale serve --http="${TAILNET_PORT}" off || true
fi
sudo tailscale serve --bg --http="${TAILNET_PORT}" "http://localhost:${LOCAL_PORT}"
...
echo "[expose-port] verified (${STATUS}). Open:"
